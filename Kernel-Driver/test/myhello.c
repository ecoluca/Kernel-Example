...
#define MYHELLO_BUFFER_READ_SIZE (4 * 1024)

static void myhello_usage(FILE *out)
{
	static const char usage_str[] = (
		"Usage:\n  myhello [options] [function] devices\n\n"
		"Functions:\n"
		...
		"Examples:\n"
		"  myhello -b -r /dev/myhello0\n"
		"  myhello -n -w \"something new\" /dev/myhello0\n"
		"  myhello -i /dev/myhello0 /dev/myhello2\n"
		"  myhello -p /dev/myhello*\n\n");
...
static void myhello_version(FILE *out)
{
	static const char prog_str[] = "myhello";
...
static int myhello_read(int dev_no, char *dev[], int nonblock)
{
...
static int myhello_write(int dev_no, char *dev[], char *buf,
...
static int myhello_ioctl(int dev_no, char *dev[], int nonblock)
{
	...
	for (i = 0; i < dev_no; i++) {
		...
		if (ioctl(fd, MYHELLO_IOCTL_SIZE_GET, &dev_size) < 0) {
			fprintf(stderr, "Can't ioctl %s\n", dev[i]);
			retval = -1;
			goto err2;
		}
		if (ioctl(fd, MYHELLO_IOCTL_FREE_GET, &dev_free) < 0) {
			fprintf(stderr, "Can't ioctl %s\n", dev[i]);
			retval = -1;
			goto err2;
		}
		if (ioctl(fd, MYHELLO_IOCTL_READY_GET, &dev_ready) < 0) {
			fprintf(stderr, "Can't ioctl %s\n", dev[i]);
			retval = -1;
			goto err2;
		}
...
static int myhello_poll(int dev_no, char *dev[], int nonblock)
{
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "rw:pibnvh", long_opts, NULL)) != -1) {
		...
		case 'n':
			...
			break;
		case 'v':
			myhello_version(stdout);
			exit(EXIT_SUCCESS);
		case 'h':
			myhello_usage(stdout);
			exit(EXIT_SUCCESS);
		default:
			myhello_usage(stderr);
			exit(EXIT_FAILURE);
		}
	}
	...
	if (opt_read) {
		retval = myhello_read(argc, argv, opt_nonblock);
	} else if (opt_write) {
		retval = myhello_write(argc, argv, arg_write, strlen(arg_write),
			opt_nonblock, opt_nonewline);
	} else if (opt_poll) {
		retval = myhello_poll(argc, argv, opt_nonblock);
	} else if (opt_info) {
		retval = myhello_ioctl(argc, argv, opt_nonblock);
	} else {
		retval = -1;
	}
